...
const DEFAULT_STATE: WalletAdapterState = { address: null, chainId: null, isConnected: false };
...
const normaliseState = (state: WalletAdapterState): WalletAdapterState => ({
  address: state.address ?? null,
  chainId: state.chainId ?? null,
  isConnected: state.isConnected ?? Boolean(state.address),
});

export function useWalletAdapterState(walletAdapter: WalletAdapter | null | undefined): WalletAdapterState {
  const [state, setState] = useState<WalletAdapterState>(() => {
    if (!walletAdapter) {
  ...
  useEffect(() => {
    ...
    const apply = (next: WalletAdapterState) => {
      if (disposed) return;
