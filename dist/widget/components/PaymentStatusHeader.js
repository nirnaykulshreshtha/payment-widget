...
export function PaymentStatusHeader({ entry, chainLookup, chainLogos }) {
    ...
    return (_jsxs("div", { className: "pw-status-header", role: "group", "aria-labelledby": `pw-status-title-${entry.id}`, children: [_jsxs("div", { className: "pw-status-header__flow", children: [_jsxs("div", { className: "pw-avatar-stack", children: [_jsx(TokenAvatar, { symbol: entry.inputToken.symbol, logoUrl: entry.inputToken.logoUrl, className: "pw-avatar--small" }), _jsx(TokenAvatar, { symbol: entry.outputToken.symbol, logoUrl: entry.outputToken.logoUrl, className: "pw-avatar--small" })] }), _jsxs("div", { className: "pw-status-header__text", "aria-label": `${originChainName} to ${destinationChainName}`, children: [_jsx("div", { className: "pw-status-header__title", id: `pw-status-title-${entry.id}`, children: paymentType }), _jsxs("div", { className: "pw-status-header__chains", children: [_jsx(ChainAvatar, { name: String(originChainName), logoUrl: chainLogos.get(entry.originChainId) }), _jsx(ArrowRight, { className: "pw-status-header__direction", "aria-hidden": "true" }), _jsx(ChainAvatar, { name: String(destinationChainName), logoUrl: chainLogos.get(entry.destinationChainId) })] })] })] }), _jsx(StatusDisplay, { status: entry.status, showOriginalStatus: false, showSimplifiedStatus: true })] }));
}
