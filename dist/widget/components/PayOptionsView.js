...
export function PayOptionsView({ options, onSelect, selectedOptionId, targetAmountLabel, targetSymbol, targetChainLabel, targetAmount, targetToken, chainLookup, chainLogos, lastUpdated, onRefresh, isRefreshing, onViewHistory, accountConnected, plannerError, }) {
    ...
    const renderNoResults = () => {
        ...
        if (searchTerm) {
            return (_jsxs("div", { className: "rounded-2xl border border-dashed border-border/60 bg-card/30 p-6 text-center", children: [_jsx("h3", { className: "text-sm font-semibold", children: "No matches found" }), _jsx("p", { className: "mt-2 text-xs text-muted-foreground", children: "Try a different asset symbol or chain name." })] }));
        }
...
function TargetSummary({ targetAmountLabel, targetSymbol, targetChainLabel, lastUpdated, onRefresh, isRefreshing }) {
    return (_jsxs("div", { className: "rounded-2xl border border-border/60 bg-card/40 p-4", children: [_jsxs("div", { className: "flex items-center justify-between text-sm", children: [_jsx("span", { className: "text-muted-foreground", children: "Target" }), _jsxs("span", { className: "font-semibold text-foreground", children: [targetAmountLabel, " ", targetSymbol, " on ", targetChainLabel] })] }), _jsxs("div", { className: "mt-2 flex items-center justify-between text-[11px] text-muted-foreground", children: [lastUpdated ? _jsxs("span", { children: ["Updated ", new Date(lastUpdated).toLocaleTimeString()] }) : _jsx("span", { children: "Ready" }), _jsxs("button", { type: "button", onClick: onRefresh, disabled: isRefreshing, className: "inline-flex items-center gap-1 text-primary underline-offset-4 hover:underline disabled:opacity-50", children: [_jsx(RefreshCw, { className: cn('h-3 w-3', isRefreshing && 'animate-spin') }), " Refresh"] })] })] }));
}
function SearchInput({ searchTerm, onSearchChange, visibleCount, totalCount }) {
    return (_jsxs("div", { className: "rounded-2xl border border-border/60 bg-card/40 p-3", children: [_jsx("input", { type: "search", value: searchTerm, onChange: (event) => onSearchChange(event.target.value), placeholder: "Search token, chain, or mode", className: "w-full rounded-xl border border-border/60 bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary" }), _jsxs("div", { className: "mt-2 text-[11px] text-muted-foreground", children: ["Showing ", visibleCount, " of ", totalCount, " options"] })] }));
}
