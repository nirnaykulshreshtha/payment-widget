...
function TransactionHashes({ entry }) {
    ...
    return (_jsx("div", { className: "space-y-3", children: _jsxs("div", { className: "grid grid-cols-1 gap-2", children: [entry.approvalTxHashes?.length ? (_jsx(TransactionGroup, { title: "Wallet approval", colorClass: "bg-yellow-500", hashes: entry.approvalTxHashes, chainId: entry.originChainId, variant: "compact" })) : null, entry.depositTxHash ? (_jsx(TransactionGroup, { title: "Deposit sent", colorClass: "bg-blue-500", hashes: [entry.depositTxHash], chainId: entry.originChainId, variant: "compact" })) : null, entry.swapTxHash ? (_jsx(TransactionGroup, { title: "Swap sent", colorClass: "bg-green-500", hashes: [entry.swapTxHash], chainId: entry.originChainId, variant: "compact" })) : null, entry.fillTxHash ? (_jsx(TransactionGroup, { title: "Funds delivered", colorClass: "bg-purple-500", hashes: [entry.fillTxHash], chainId: entry.destinationChainId, variant: "compact" })) : null, entry.wrapTxHash ? (_jsx(TransactionGroup, { title: "Wrap step", colorClass: "bg-orange-500", hashes: [entry.wrapTxHash], chainId: entry.originChainId, variant: "compact" })) : null] }) }));
}
function UpdatedFooter({ updatedAt }) {
    return (_jsxs("div", { className: "flex items-center justify-between border-t border-border/30 pt-3", children: [_jsxs("div", { className: "flex items-center gap-2 text-muted-foreground", children: [_jsx(ClockIcon, { className: "h-3 w-3" }), _jsx("span", { className: "text-[10px] uppercase tracking-wide", children: "Last updated" })] }), _jsx("time", { className: "text-[10px] text-muted-foreground/80", children: new Date(updatedAt).toLocaleString() })] }));
}
